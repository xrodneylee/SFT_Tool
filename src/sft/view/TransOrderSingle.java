...
public class TransOrderSingle extends JPanel implements ActionListener{
	ConnectDB conn;
	...
	public TransOrderSingle(JTabbedPane jtb,ConnectDB conn){
		this.conn = conn;
		this.jtb =jtb;
		setLayout(null);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(!CMOIDStart.getText().equals("") && CMOIDEnd.getText().equals("")){
			...
			condition = " AND STL.KEYID BETWEEN '"+CMOIDStart.getText()+"' AND '"+CMOIDEnd.getText()+"'";
		}
		if(cmd.equals("Verify")){
			TransOrderSingleDetail tosd = new TransOrderSingleDetail(conn,condition);
			jtb.addTab("移轉個數比對明細", tosd);
			jtb.setSelectedComponent(tosd);
		}
	}
}
